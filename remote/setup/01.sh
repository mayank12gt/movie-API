...
set -eu
# 
#scp -i ./movie-backend-key.pem -r #C:\Temp\movie_seed.csv  greenlight@13.#53.193.190:/home/greenlight/
#==================================================================================== #
# VARIABLES
# ==================================================================================== #
...
#rsync --archive --chown=greenlight:gre
#enlight .ssh /home/greenlight
...
sudo -i -u postgres psql -c "CREATE DATABASE moviedb"
sudo -i -u postgres psql -d moviedb -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d moviedb -c "CREATE ROLE movieuser WITH LOGIN PASSWORD '${DB_PASSWORD}'"
# Add a DSN for connecting to the greenlight database to the system-wide environment
# variables in the /etc/environment file.
echo "GREENLIGHT_DB_DSN='postgres://movieuser:${DB_PASSWORD}@localhost/moviedb?sslmode=disable'" >> /etc/environment
